...
class Auth extends CI_Controller
{
	...
	private function _sendEmail($token, $type)
	{
		...
		$config['smtp_user'] = 'cchikadamayanti29@gmail.com'; //email yg dirubah keamanannya
		$config['smtp_pass'] = 'nuchklecsddkvegi';
		...
		$this->email->from('cchikadamayanti29@gmail.com');
	...
	public function resetpassword()
	{
		...
		if ($user) {
			...
			if ($user_token) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Reset Password failed!
				Wrong token!</div>');
				redirect('auth/forgotPassword');
			}
		} else {
			$this->session->set_falshdata('message', '<div class="alert alert-danger" role="alert">Reset Password failed!
			Wrong email!</div>');
			redirect('auth/forgotPassword');
		}
	}
	public function changePassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="aleet alert-success" role="alert">Password has been change!Please login</div>');
			redirect('auth');
		}
	}
}
